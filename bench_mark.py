...
def cmd(command,result_dict,file):
    ...
    try:
        subp.wait(600)
            ...
            result_dict[file]["Fast-BFS search completed in"] = "--"
            result_dict[file]["Goals found"] = "--"
            result_dict[file]["Goals_Edges found"] = "--"
            result_dict[file]["Plan found with cost"] = "no plan"
    if subp.poll() == 0:
        for line in (subp.communicate()[0]).split("\n"):
            ...
            if "Goals found" in line:
                result_dict[file]["Goals found"] = line.split(":")[-1]
            if "Goals_Edges found" in line:
                result_dict[file]["Goals_Edges found"] = line.split(":")[-1]
            if "Plan found with cost" not in result_dict[file]:
                result_dict[file]["Plan found with cost"] = "no plan"
    else:
        ...
        result_dict[file]["Fast-BFS search completed in"] = "time out"
        result_dict[file]["Goals found"] = "time out"
        result_dict[file]["Goals_Edges found"] = "time out"
        result_dict[file]["Plan found with cost"] = "no plan"
    return result_dict
...
def write(output_list, output_file):
    ...
    with open(output_file, "w") as f:
        writer = csv.writer(f)
        writer.writerow(["file name", "Goals found","Goals_Edges found","Plan found with cost", "Total time", "Nodes generated during search", "Nodes expanded during search",
                  "Fast-BFS search completed in"])
        for row in output_list:
            writer.writerow(row)

def run_bfws(domain):
    subprocess.Popen("mkdir /home/ubuntu/new-sample-pddl-results/"+domain, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding="utf-8")
    ...
    for file in files:
        print(file)
        cmd("/xenon/BFWS_Backward_Cmake/cmake-build-debug/bfws --domain "+ domain_path+" --problem "+os.path.join(path,file)+" --output /home/ubuntu/new-sample-pddl-results/"+domain+"/" + file + "_result.txt --max_novelty 2 --k-BFWS true",result_dict,file)
    result_out_put = {}
    ...
    for key in result_dict_keys:
        temp_list = []
        for i in ["Goals found", "Goals_Edges found", "Plan found with cost", "Total time",
    ...
    write(out_put_list,os.path.join("/home/ubuntu/new-sample-pddl-results",domain,"result.csv"))
